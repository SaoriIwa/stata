...
{p 7 7 2} In the first simulation (at the "0" (0%) on the x axis in the produced graph), {cmdab:manskiBounds} produces the "extreme" bounds: all missing observations in outcome are assigned the value that would produce the furtherst opposite effect from the original regression result. For example, if the original regression coefficient of the treatment was negative, all missing will be assigned positive value. This is then relaxed by varying the "bounding fraction" until outcome simulation is random in both treatment and control groups with missing outcomes (50%). At 50%, the etimate induces measurement error in the original estimate proportional to the number of missing outcome values. The displayed graph shows this relaxation process, as well as the bounding fractions at which p<0.01 and p<0.05 are attained, and the estimated effect from the original model on non-missing data.{p_end}
